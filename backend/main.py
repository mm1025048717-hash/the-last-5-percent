...
# Agent instances
denoise_agent: DenoiseAgent | None = None
...
async def lifespan(app: FastAPI):
    ...
    global denoise_agent, scenario_agent, history_agent
...
app = FastAPI(
    ...
    description="AI驱动的产品避坑分析工具，专注于告诉你「为什么不该买」",
    version="1.0.0",
    lifespan=lifespan
)
...
def generate_alternatives(product_name: str, defects: list) -> list[AlternativeProduct]:
    """生成替代方案建议"""
    # 这里是演示数据，实际应该基于产品类别和缺陷动态生成
    alternatives_db = {
        "扫地机器人": [
            AlternativeProduct(
                name="石头 G20",
                price_range="3000-3500元",
                advantage="全自动洗拖布，彻底解决边刷缠绕问题",
                solved_defects=["边刷缠绕", "清洁效果"],
                link=None
            ),
            AlternativeProduct(
                name="追觅 S10 Pro",
                price_range="2500-3000元",
                advantage="AI视觉避障，不会误触宠物粪便",
                solved_defects=["避障能力", "建图准确性"],
                link=None
            ),
            AlternativeProduct(
                name="云鲸 J4",
                price_range="3500-4000元",
                advantage="分体式设计，维护简单，电机5年质保",
                solved_defects=["电机耐久性", "维护成本"],
                link=None
            )
        ],
        "default": [
...
async def analyze_product(request: AnalysisRequest):
    ...
    这是核心 API，会串联三个 Agent 进行深度分析：
    1. Agent A: 差评脱水机 - 过滤噪音，提取真实缺陷
    2. Agent B: 场景撞墙预测 - 基于使用场景预测问题
    3. Agent C: 黑历史追溯 - 挖掘产品/品牌历史问题
    """
    ...
    denoise_task = denoise_agent.execute({
        "product_name": product_name,
        "reviews": []  # 演示模式，使用内置数据
    })
    
    scenario_task = scenario_agent.execute({
        "product_name": product_name,
        "user_scenario": request.user_scenario or ""
    })
    
    history_task = history_agent.execute({
        "product_name": product_name
    })
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "agents": {
            "denoise": denoise_agent is not None,
            "scenario": scenario_agent is not None,
            "history": history_agent is not None
        }
    }
